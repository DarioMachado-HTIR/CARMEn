...
class LocalPlanner(object):
    ...
    def set_global_plan(self, current_plan, stop_waypoint_creation=True, clean_queue=True, draw=False):
        """
        ...
        if draw:
            for wp in self._waypoints_queue:
                self._world.debug.draw_string(wp[0].transform.location, '^', draw_shadow=False,
                    color=carla.Color(r=0, g=0, b=225), life_time=15.0,
        ...
        for waypoint, _ in self._waypoints_queue:
            ...
            if len(self._waypoints_queue) - num_waypoint_removed == 1:
                ...
                if self._offset is not None:
                    min_distance = abs(self._offset) + 1 
                else:
                    min_distance = 1
            else:
